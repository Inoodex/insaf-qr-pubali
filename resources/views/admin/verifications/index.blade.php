...
@section('page_title', 'Account & Statement Verifications')
        ...
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-header bg-transparent border-bottom py-3 px-4 d-flex justify-content-between align-items-center">
                <h5 class="card-title fw-bold mb-0 {{ $editVerification ? 'text-warning-emphasis' : 'text-primary' }}">
                    ...
                    {{ $editVerification ? 'Edit Verification (' . $editVerification->account_name . ')' : 'Solvency Certificate & Bank Statement Details' }}
                </h5>
                ...
                <div class="row g-4 align-items-start">
                    ...
                    <div class="col-lg-7">
                        <form action="{{ $editVerification ? route('admin.verifications.update', $editVerification->id) : route('admin.verifications.store') }}" method="POST" id="unifiedForm">
                            ...
                            <!-- Section 1: Common Account & Document Details -->
                            <div class="p-3 bg-body-secondary bg-opacity-50 rounded-3 mb-3 border">
                                <h6 class="fw-bold mb-3 text-body d-flex align-items-center">
                                    <i class="bi bi-person-badge-fill me-2 text-primary"></i>1. Account & Document Information
                                </h6>
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <label class="form-label fw-bold small text-muted text-uppercase">Account No <span class="text-danger">*</span></label>
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label fw-bold small text-muted text-uppercase">Account Type <span class="text-muted fw-normal">(Optional)</span></label>
                                        <div class="input-group input-group-md">
                                            <span class="input-group-text"><i class="bi bi-tag"></i></span>
                                            <input type="text" name="account_type" id="inputAccountType" class="form-control @error('account_type') is-invalid @enderror" value="{{ old('account_type', $editVerification->account_type ?? ($activeVerification->account_type ?? 'SAVINGS BANK ACCOUNT')) }}">
                                            @error('account_type')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label fw-bold small text-muted text-uppercase">Certificate ID / Ref No <span class="text-muted fw-normal">(Optional)</span></label>
                                        <div class="input-group input-group-md">
                                            <span class="input-group-text"><i class="bi bi-hash"></i></span>
                                            <input type="text" name="certificate_id" id="inputCertId" class="form-control font-monospace fw-bold @error('certificate_id') is-invalid @enderror" value="{{ old('certificate_id', $editVerification->certificate_id ?? ($activeVerification->certificate_id ?? '')) }}">
                            ...
                            <div class="row g-3">
                                ...
                                <!-- Section 2: Statement Details (Statement QR) -->
                                <div class="col-md-6">
                                    <div class="p-3 rounded-3 border h-100" style="background-color: rgba(16, 185, 129, 0.03); border-color: rgba(16, 185, 129, 0.2) !important;">
                                        <h6 class="fw-bold mb-3 text-success d-flex align-items-center">
                                            <i class="bi bi-file-earmark-text-fill me-2"></i>2. Bank Statement Section (Statement QR)
                                        </h6>
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group input-group-md">
                                                ...
                                                <input type="text" name="opening_balance" id="inputOpenBal" class="form-control font-monospace fw-bold @error('opening_balance') is-invalid @enderror" value="{{ old('opening_balance', $editVerification ? $editVerification->formatted_opening_balance : ($activeVerification ? $activeVerification->formatted_opening_balance : '')) }}" required>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group input-group-md">
                                                <span class="input-group-text fw-bold text-success">৳</span>
                                                <input type="text" name="closing_balance" id="inputCloseBal" class="form-control font-monospace fw-bold @error('closing_balance') is-invalid @enderror" value="{{ old('closing_balance', $editVerification ? $editVerification->formatted_closing_balance : ($activeVerification ? $activeVerification->formatted_closing_balance : '')) }}" required>
                                        ...
                                        <div class="row g-2 mb-3">
                                            <div class="col-6">
                                                <label class="form-label fw-bold small text-muted text-uppercase">Period From</label>
                                                <input type="date" name="statement_period_from" id="inputPeriodFrom" class="form-control form-control-sm fw-bold @error('statement_period_from') is-invalid @enderror" value="{{ old('statement_period_from', isset($editVerification) && $editVerification->statement_period_from ? $editVerification->statement_period_from->format('Y-m-d') : (isset($activeVerification) && $activeVerification->statement_period_from ? $activeVerification->statement_period_from->format('Y-m-d') : date('Y-m-d', strtotime('-3 months')))) }}">
                                            </div>
                                            <div class="col-6">
                                                <label class="form-label fw-bold small text-muted text-uppercase">Period To</label>
                                                <input type="date" name="statement_period_to" id="inputPeriodTo" class="form-control form-control-sm fw-bold @error('statement_period_to') is-invalid @enderror" value="{{ old('statement_period_to', isset($editVerification) && $editVerification->statement_period_to ? $editVerification->statement_period_to->format('Y-m-d') : (isset($activeVerification) && $activeVerification->statement_period_to ? $activeVerification->statement_period_to->format('Y-m-d') : date('Y-m-d'))) }}">
                                            </div>
                                        </div>

                                        <!-- Statement Generated Date & Time -->
                                        <div>
                                            <label class="form-label fw-bold small text-muted text-uppercase">Statement Generated Date & Time</label>
                                            <div class="input-group input-group-sm">
                                                <span class="input-group-text"><i class="bi bi-clock"></i></span>
                                                <input type="datetime-local" step="1" name="statement_generated_at" id="inputStmtGenAt" class="form-control fw-bold @error('statement_generated_at') is-invalid @enderror" value="{{ old('statement_generated_at', isset($editVerification) && $editVerification->statement_generated_at ? $editVerification->statement_generated_at->format('Y-m-d\TH:i:s') : (isset($activeVerification) && $activeVerification->statement_generated_at ? $activeVerification->statement_generated_at->format('Y-m-d\TH:i:s') : date('Y-m-d\TH:i:s'))) }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <!-- Section 3: Solvency Certificate Details (Solvency QR) -->
                                <div class="col-md-6">
                                    <div class="p-3 rounded-3 border h-100" style="background-color: rgba(79, 70, 229, 0.03); border-color: rgba(79, 70, 229, 0.2) !important;">
                                        ...
                                        <div class="row g-2 mb-3">
                                            <div class="col-6">
                                                <label class="form-label fw-bold small text-muted text-uppercase">A/C Open Date</label>
                                                <input type="date" name="account_open_date" id="inputOpenDate" class="form-control form-control-sm fw-bold @error('account_open_date') is-invalid @enderror" value="{{ old('account_open_date', isset($editVerification) && $editVerification->account_open_date ? $editVerification->account_open_date->format('Y-m-d') : (isset($activeVerification) && $activeVerification->account_open_date ? $activeVerification->account_open_date->format('Y-m-d') : '')) }}">
                                            </div>
                                            <div class="col-6">
                                                <label class="form-label fw-bold small text-muted text-uppercase">Issue Date</label>
                                                <input type="date" name="report_generation_date" id="inputReportDate" class="form-control form-control-sm fw-bold @error('report_generation_date') is-invalid @enderror" value="{{ old('report_generation_date', isset($editVerification) && $editVerification->report_generation_date ? $editVerification->report_generation_date->format('Y-m-d') : (isset($activeVerification) && $activeVerification->report_generation_date ? $activeVerification->report_generation_date->format('Y-m-d') : date('Y-m-d'))) }}">
                                            </div>
                                        </div>

                                        <div class="row g-2">
                                            <div class="col-6">
                                                <label class="form-label fw-bold small text-muted text-uppercase">Foreign Currency</label>
                                                <input type="text" name="currency" id="inputCurrency" class="form-control form-control-sm font-monospace text-uppercase fw-bold @error('currency') is-invalid @enderror" value="{{ old('currency', $editVerification->currency ?? ($activeVerification->currency ?? '')) }}">
                                            </div>
                                            <div class="col-6">
                                                <label class="form-label fw-bold small text-muted text-uppercase">Equivalent Balance</label>
                                                <input type="text" name="equivalent_balance" id="inputEqBal" class="form-control form-control-sm font-monospace fw-bold @error('equivalent_balance') is-invalid @enderror" value="{{ old('equivalent_balance', $editVerification->equivalent_balance ?? ($activeVerification->equivalent_balance ?? '')) }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="col-lg-5 border-start ps-lg-4">
                        ...
                        <ul class="nav nav-pills nav-fill mb-3" id="qrTabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active py-2 fw-semibold small" id="cert-tab" data-bs-toggle="tab" data-bs-target="#certQrTabPane" type="button" role="tab">
                                    <i class="bi bi-patch-check-fill me-1 text-primary"></i>Solvency QR
                                </button>
                            </li>
                            <li class="nav-item" role="presentation">
                        ...
                        <div class="tab-content" id="qrTabsContent">
                            ...
                            <!-- Tab 1: Solvency Certificate QR -->
                            <div class="tab-pane fade show active text-center" id="certQrTabPane" role="tabpanel">
                                <div class="qr-preview-box shadow-xs mb-3 mx-auto d-flex flex-column align-items-center justify-content-center p-3" style="max-width: 260px; min-height: 240px; background: #ffffff; border-radius: 12px; border: 1px solid rgba(0,0,0,0.08);">
                                ...
                                <button type="button" class="btn btn-outline-primary btn-sm fw-bold w-100 {{ ($activeVerification || $editVerification) ? '' : 'd-none' }}" id="btnDownloadCertQr" onclick="downloadCurrentCertQR()">
                                    <i class="bi bi-download me-1"></i> Save Solvency QR (PNG)
                                </button>
                            </div>
                            ...
                            <div class="tab-pane fade text-center" id="stmtQrTabPane" role="tabpanel">
                                ...
                                <button type="button" class="btn btn-outline-success btn-sm fw-bold w-100 {{ ($activeVerification || $editVerification) ? '' : 'd-none' }}" id="btnDownloadStmtQr" onclick="downloadCurrentStmtQR()">
                                    <i class="bi bi-download me-1"></i> Save Statement QR (PNG)
                                </button>
                            </div>
            ...
            <div class="card-header bg-transparent border-bottom py-3 px-4 d-flex align-items-center justify-content-between flex-nowrap">
                ...
                <div class="card-tools ms-auto d-flex flex-nowrap align-items-center gap-2">
                    <div class="input-group input-group-sm" style="width: 260px; flex-shrink: 0;">
                        ...
                        <input type="text" id="searchVerifInput" class="form-control border-start-0 ps-2" onkeyup="filterVerifTable()">
                    </div>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0 small">
                        <thead>
                            <tr>
                                <th class="ps-4">Account No & Name</th>
                        ...
                        <tbody id="verifTableBody">
                            ...
                            <tr style="cursor: pointer;" 
                                ...
                                onclick="loadRow(
                                    '{{ $v->account_no }}',
                                    '{{ addslashes($v->account_name) }}',
                                    '{{ addslashes($v->account_type ?? '') }}',
                                    '{{ $v->certificate_id ?? '' }}',
                                    '{{ $v->formatted_certificate_balance }}',
                                    '{{ $v->formatted_opening_balance }}',
                                    '{{ $v->formatted_closing_balance }}',
                                    '{{ $v->statement_period_from ? $v->statement_period_from->format('Y-m-d') : '' }}',
                                    '{{ $v->statement_period_to ? $v->statement_period_to->format('Y-m-d') : '' }}',
                                    '{{ $v->statement_generated_at ? $v->statement_generated_at->format('Y-m-d\TH:i:s') : '' }}',
                                    '{{ $v->account_open_date ? $v->account_open_date->format('Y-m-d') : '' }}',
                                    '{{ $v->report_generation_date ? $v->report_generation_date->format('Y-m-d') : '' }}',
                                ...
                                <td class="ps-4">
                                    ...
                                    <span class="font-monospace text-primary fw-semibold">{{ $v->account_no }}</span>
                                    @if($v->account_type)
                                    <span class="badge bg-secondary-subtle text-secondary font-monospace" style="font-size: 0.72rem;">{{ $v->account_type }}</span>
                                    @endif
                                </td>
                                <td>
                                    @if($v->certificate_id)
                                        <span class="badge bg-light text-dark border font-monospace">{{ $v->certificate_id }}</span>
                                    @else
                                        <span class="text-muted">&mdash;</span>
                                    @endif
                                </td>
                                <td>
                                    <span class="badge bg-primary-subtle text-primary border border-primary-subtle font-monospace fs-7">
                                        ৳{{ ltrim($v->formatted_certificate_balance, '৳ ') }}
                                    </span>
                                </td>
                                <td>
                                    <div class="small text-nowrap">Open: <strong class="text-secondary font-monospace">৳{{ ltrim($v->formatted_opening_balance, '৳ ') }}</strong></div>
                                    <div class="small text-nowrap">Close: <strong class="text-success font-monospace">৳{{ ltrim($v->formatted_closing_balance, '৳ ') }}</strong></div>
                                </td>
                                <td>
                                    <span class="text-nowrap fw-medium">{{ $v->statement_period }}</span>
                                </td>
                                <td>
                                    <span class="text-nowrap small text-muted">{{ $v->formatted_statement_generated_at }}</span>
                                </td>
                                <td class="text-end pe-4" onclick="event.stopPropagation()">
                                    <div class="d-inline-flex align-items-center gap-1">
                                        ...
                                        <div class="dropup d-inline-block">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end shadow-lg border py-2" style="font-size: 0.85rem; min-width: 230px; z-index: 9999;">
                                                <li>
                                                    <button type="button" class="dropdown-item d-flex align-items-center py-2" onclick="downloadRowQR('cert', '{{ $v->certificate_verification_url }}', '{{ $v->account_no }}')">
                                                        ...
                                                        <span>Download Solvency QR</span>
                                                    </button>
                                                </li>
                                                <li>
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="text-center py-4 text-muted">No account records saved yet. Fill in the form above and click Save.</td>
                            </tr>
                            @endforelse
...
<script>
    ...
    // 2. Balance inputs: manual formatting, only numbers, comma and decimal point
    ['inputCertBal', 'inputOpenBal', 'inputCloseBal', 'inputEqBal'].forEach(id => {
    ...
    // 3. Auto-mirror Closing Balance to Solvency / Certificate Balance
    const closeBalEl = document.getElementById('inputCloseBal');
    const certBalEl = document.getElementById('inputCertBal');
    if (closeBalEl && certBalEl) {
        closeBalEl.addEventListener('input', function() {
            certBalEl.value = this.value;
            renderBothQRs();
        });
    }
    ...
    function getPayloads() {
        return { 
            certPayload: currentCertUrl || '', 
            stmtPayload: currentStmtUrl || '' 
        };
    }
    ...
    function renderQR(type, payload) {
        ...
        if (!payload) {
            holder.innerHTML = `<div class="text-muted small py-4"><i class="bi bi-qr-code fs-1 d-block text-secondary mb-2 opacity-50"></i>Save record to generate ${type === 'cert' ? 'Solvency' : 'Statement'} QR</div>`;
    ...
    // Dynamic typing preview listeners
    ['inputAccountNo', 'inputAccountName', 'inputAccountType', 'inputCertId', 'inputCertBal', 'inputOpenBal', 'inputCloseBal', 'inputPeriodFrom', 'inputPeriodTo', 'inputStmtGenAt'].forEach(id => {
        const el = document.getElementById(id);
    ...
    async function downloadRowQR(type, qrUrl, accNo) {
        if (!qrUrl) return;
        const fileName = `Pubali_${type === 'cert' ? 'Certificate' : 'Statement'}_QR_${accNo || 'Record'}_${Date.now()}`;
    ...
    function loadRow(accNo, accName, accType, certId, certBal, openBal, closeBal, periodFrom, periodTo, stmtGenAt, openDate, reportDate, currency, eqBal, certUrl, stmtUrl) {
        ...
        document.getElementById('inputPeriodFrom').value = periodFrom || '';
        document.getElementById('inputPeriodTo').value = periodTo || '';
        document.getElementById('inputStmtGenAt').value = stmtGenAt || '';
        const openDateInput = document.getElementById('inputOpenDate');
        if (openDateInput) openDateInput.value = openDate || '';
        const reportDateInput = document.getElementById('inputReportDate');
        if (reportDateInput) reportDateInput.value = reportDate || '';
        const currencyInput = document.getElementById('inputCurrency');
        if (currencyInput) currencyInput.value = currency || '';
        const eqBalInput = document.getElementById('inputEqBal');
        if (eqBalInput) eqBalInput.value = eqBal || '';
    ...
    function filterVerifTable() {
        ...
        if (resetBtn) {
            resetBtn.style.display = (textFilter.length > 0) ? 'inline-block' : 'none';
        }
