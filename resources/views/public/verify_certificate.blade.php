...
<body class="d-flex flex-column min-vh-100">
    ...
    <main class="flex-grow-1">
        <div class="container my-3 my-md-4 px-2 px-sm-3">
            <div class="container py-2 py-md-3 px-2 px-sm-3">
                    ...
                    <div class="col-12 col-md-10 col-lg-9">
                        <div class="card shadow-sm p-3 p-sm-4 bg-white border-0 card-box">
                            ...
                            <div class="text-center mb-3 mb-md-4">
                                <h4 class="fw-bold text-decoration-underline text-uppercase fs-5 fs-md-4 certificate-title">
                                    TO WHOM IT MAY CONCERN
                                </h4>
                            </div>

                            <!-- 2. Header Info -->
                            <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-start border-bottom pb-3 mb-3 gap-2">
                                <!-- Left Column: Account Holder & Certificate ID -->
                                <div class="d-flex flex-column gap-2">
                                    <div>
                                        <small class="text-muted d-block text-uppercase">ACCOUNT HOLDER NAME:</small>
                                        <span class="fw-bold text-uppercase text-dark">{{ strtoupper($certificate->account_name) }}</span>
                                    </div>
                                    <div>
                                        <small class="text-muted d-block text-uppercase">CERTIFICATE ID:</small>
                                        <span class="fw-bold text-dark">{{ $certificate->certificate_id ?? '' }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="text-sm-end">
                                    ...
                                    <span class="fw-bold text-dark">
                                        {{ $certificate->report_generation_date ? $certificate->report_generation_date->format('d-M-Y') : ($certificate->formatted_generation_date ?: date('d-M-Y')) }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="table-responsive mb-2">
                                <table class="table table-bordered table-sm table-hover align-middle text-center mb-0">
                                    <thead class="table-light">
                                        <tr class="small text-uppercase">
                                            ...
                                            <th>A/C Type</th>
                                            <th>Balance (BDT)</th>
                                            @if(!empty($certificate->currency) && strtoupper($certificate->currency) !== 'BDT')
                                            <th>Balance ({{ strtoupper($certificate->currency) }})</th>
                                            @endif
                                        </tr>
                                    </thead>
                                    <tbody class="small">
                                        <tr>
                                            <td>1</td>
                                            <td class="fw-normal text-nowrap">
                                                @php
                                                    $rawAcc = (string)($certificate->formatted_account_no ?? $certificate->account_no);
                                            ...
                                            <td class="text-nowrap">
                                                {{ $certificate->account_open_date ? $certificate->account_open_date->format('d-M-Y') : ($certificate->report_generation_date ? $certificate->report_generation_date->format('d-M-Y') : date('d-M-Y')) }}
                                            </td>
                                            <td class="text-start">
                                                {{ ucwords(strtolower($certificate->account_type ?: 'Savings Bank Account')) }}
                                            </td>
                                            <td class="text-end fw-bold text-nowrap">
                                                <span>৳{{ is_numeric(str_replace(',', '', (string)($certificate->report_date_balance ?? $certificate->formatted_balance))) ? number_format((float) str_replace(',', '', (string)($certificate->report_date_balance ?? $certificate->formatted_balance)), 2) : ltrim($certificate->formatted_balance, '৳ ') }}</span>
                                            </td>
                                            @if(!empty($certificate->currency) && strtoupper($certificate->currency) !== 'BDT')
                                            <td class="text-end fw-bold text-nowrap">
                                                <span>{{ is_numeric(str_replace(',', '', (string)$certificate->equivalent_balance)) ? number_format((float) str_replace(',', '', (string)$certificate->equivalent_balance), 2) : ($certificate->equivalent_balance ?? '') }}</span>
                                            </td>
                                            @endif
                                        </tr>
                                    </tbody>
                                    <tfoot class="fw-bold table-light small">
                                        <tr>
                                            <td colspan="4" class="text-end">Total</td>
                                            <td class="text-end text-nowrap">
                                                <span class="text-success">৳{{ is_numeric(str_replace(',', '', (string)($certificate->report_date_balance ?? $certificate->formatted_balance))) ? number_format((float) str_replace(',', '', (string)($certificate->report_date_balance ?? $certificate->formatted_balance)), 2) : ltrim($certificate->formatted_balance, '৳ ') }}</span>
                                            </td>
                                            @if(!empty($certificate->currency) && strtoupper($certificate->currency) !== 'BDT')
                                            <td class="text-end text-nowrap">
                                                <span class="text-success">{{ is_numeric(str_replace(',', '', (string)$certificate->equivalent_balance)) ? number_format((float) str_replace(',', '', (string)$certificate->equivalent_balance), 2) : ($certificate->equivalent_balance ?? '') }}</span>
                                            </td>
                                            @endif
                                        </tr>
                                    </tfoot>
